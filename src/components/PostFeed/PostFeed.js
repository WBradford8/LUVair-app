...
export const PostFeed = () => {
  ...
  const fetchPosts = () => {
    fetch(
      `http://localhost:8088/posts?&_expand=flight&_expand=user&_embed=comments`
    )
      .then((res) => res.json())
      .then((postArray) => {
        setPosts(postArray);
  ...
  return (
    ...
    <div className="headerContainer">
      ...
      <div className="postFlightContainer">
      {posts
        .slice()
        .reverse()
        .map((postObject) => {
          ...
          return (
            <div className="postedFlight">
              ...
              <p>
                <b>Poster's thoughts:</b> {postObject.summary}
              </p>
              <p>
              ...
              <div className="commentSection">
                {comments.map((commentObject) => {
                  if (commentObject.postId === postObject.id) {
                    return (
                      <p>{`Posted by ${commentObject.user.name} ${commentObject.comment}`}</p>
                    );
                  }
                })}
              </div>
